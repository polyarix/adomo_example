                    ...
                    if (!$(obj).hasClass("select2-hidden-accessible"))
                    {
                        ...
                        $obj.on("select2:select", function(e) {
                            if(e.params.data.isNew){
                                axios.post('/admin/category/tags', { name: e.params.data.text })
                                    .then(function(res) {
                                        const data = res.data;

                                        console.log($(this).find('[value="'+e.params.data.id+'"]'));

                                        $obj.find('[value="'+e.params.data.id+'"]').replaceWith('<option selected value="'+data.id+'">'+data.name+'</option>');
                                    })
                            }
                        });
