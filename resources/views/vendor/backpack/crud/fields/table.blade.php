...
<?php
    ...
    $sortEnabled = !(isset($field['sort_enabled']) && $field['sort_enabled'] === false);
...
<div ng-app="backPackTableApp" ng-controller="tableController" @include('crud::inc.field_wrapper_attributes') >
    ...
    <div class="array-container form-group">
        ...
        <table class="table table-bordered table-striped m-b-0" ng-init="field = '#{{ $field['name'] }}'; items = {{ $items }}; max = {{$max}}; min = {{$min}}; sortEnabled = {{ $sortEnabled ? 'true' : 'false' }}; maxErrorTitle = '{{trans('backpack::crud.table_cant_add', ['entity' => $item_name])}}'; maxErrorMessage = '{{trans('backpack::crud.table_max_reached', ['max' => $max])}}'; emptyRow='{{ $empty_row }}';">
            ...
            <tbody ui-sortable="sortableOptions" ng-model="items" class="table-striped">
                ...
                <tr ng-repeat="item in items" class="array-row">
                    ...
                    @endforeach
                    <td ng-if="(max == -1 || max > 1) && sortEnabled">
