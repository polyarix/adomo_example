    ...
    <div class="well well-sm file-preview-container">
    	@foreach($values as $file)
    		...
    		<div class="file-preview">
		        ...
		        @else
		            <a target="_blank" href="{{ isset($field['disk'])?asset(\Storage::disk($field['disk'])->url($file_path)):asset($storage_path) }}">{{ $file_path }}</a>
		        @endif
		    	<a id="{{ $field['name'] }}_{{ $file->id }}_clear_button" href="#" class="btn btn-default btn-xs pull-right file-clear-button" title="Clear file" data-filename="{{ $file->id }}"><i class="fa fa-remove"></i></a>
