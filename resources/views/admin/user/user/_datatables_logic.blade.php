    ...
    var crud = {
      ...
      dataTableConfiguration: {
        ...
        createdRow: function( row, data, dataIndex) {
            let documentsCount = $(row).find('[data-documents]');

            if(!documentsCount || !documentsCount.data('documents')) {
                return;
            }

            /*let entityStatus = $(row).find('[data-status]');
            if(!entityStatus || !entityStatus.data('status')) {
                return;
            }
            let status = entityStatus.data('status');
            row.classList.add(window.statusClasses[status])*/

            let count = +documentsCount.data('documents');

            if(count > 0) {
                row.classList.add(window.documentsClass)
            }
        },
